...
Use the repository convention `ds^2=-dt^2+(dx-v(t)f dt)^2+dy^2+dz^2`, unit lapse and flat spatial metric. The momentum constraint gives

`j_x = -v (f_yy+f_zz)/(16 pi)`.

At fixed spatial coordinates, differentiating the explicit velocity factor gives the acceleration-only contribution

`(partial_t j_x)_a = -a (f_yy+f_zz)/(16 pi)`.

The v0.3.5 acceleration-only stress tensor has `S_xx=0`, `S_xy=a f_y/(16 pi)`, `S_xz=a f_z/(16 pi)`, hence

`(partial_j S_xj)_a = +a (f_yy+f_zz)/(16 pi)`.

Therefore the acceleration-order local momentum conservation residual cancels pointwise:
