...
## 2026-09-16 19:26 UTC — local acceleration power interpretation guard
The equality between integrated local Eulerian acceleration terms and `dE_total/dt` closes the prescribed Einstein-source budget only. It does not establish that an apparatus can create the required stress-energy, nor that the signed inventory derivative equals wall-plug power. Symmetric off-diagonal tensor terms must be counted twice in `K_ij S^ij`; omitting that factor would falsely produce a factor-of-two mismatch.

## 2026-09-16 20:27 UTC — system-boundary control
Photon pressure is only a net propulsion/reaction baseline when momentum crosses the chosen system boundary. A source and beam dump/mirror both mounted on the same closed balance can exchange internal momentum without net steady external thrust. Future 'reactionless' candidates must define the complete boundary and account for radiation, gas, thermal, electromagnetic and mechanical momentum leakage before any anomaly claim.

## 2026-09-16 21:29 UTC — archive-root invocation error
Running pytest one directory above the materialized archive's actual repository root caused 17 import collection errors (`ModuleNotFoundError: src...`). Archive contained an extra `repo/` nesting level. Corrected working directory; unchanged v0.3.14 suite then passed 90/90. No scientific result depended on the failed invocation.

## 2026-09-17 01:27 UTC — artifact-floor interpretation guard
Do not call `B_art+5 sigma_art` a universal physical detection limit. It is a deliberately conservative, predeclared project eligibility rule. Systematic artifact bounds must be measured in matched nulls; they are not inferred from NASA's historical percentage tares. Unknown-sign systematics are summed rather than allowed to cancel. Passing this gate establishes measurement eligibility only, not propulsion or anomaly evidence.
