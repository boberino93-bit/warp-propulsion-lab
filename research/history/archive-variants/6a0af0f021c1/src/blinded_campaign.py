...
def campaign_threshold_n(sigma_art_n: float, systematic_bound_n: float, samples_per_polarity: int, z: float = 5.0) -> float:
    ...
    # Two independent polarity groups are sign-aligned then averaged.
...
def _one_trial(force_n: float, sigma_n: float, bias_n: float, n: int, rng: random.Random) -> float:
    # Balanced + and - runs. Reversal is sign-aligned before averaging.
    plus = [force_n + bias_n + rng.gauss(0.0, sigma_n) for _ in range(n)]
    minus = [-force_n + bias_n + rng.gauss(0.0, sigma_n) for _ in range(n)]
    # Sign alignment cancels any common, non-reversing bias in expectation.
...
def monte_carlo_campaign(injected_force_n: float, sigma_art_n: float, systematic_bound_n: float,
    ...
    """Estimate detection and worst-bound null false-positive rates with frozen rules.

    Detection requires the sign-aligned estimate to exceed the predeclared threshold.
    The null simulation includes a common-mode bias at the systematic bound; polarity
    reversal cancels that non-reversing term by construction, which is itself a required
    experimental diagnostic rather than an assumption about all artifacts.
    """
...
def randomized_blind_schedule(repeats: int = 16, seed: int = 319):
    """Return reproducible randomized hidden-condition schedule for preregistration.

    Conditions are balanced ON+, ON-, OFF and SHAM. In a real campaign the operator
    should receive only opaque run IDs; the condition key remains withheld until the
    analysis code and thresholds are frozen.
    """
    if repeats < 1:
